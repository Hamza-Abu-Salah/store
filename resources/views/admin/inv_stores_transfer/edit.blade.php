                        ...
                        @if ($data['is_approved'] == 0)
                            <form action="{{ route('admin.inv_stores_transfer.update', $data['id']) }}" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    <select @if ($added_counter_details > 0) disabled @endif name="transfer_from_store_id"
                                        id="transfer_from_store_id" class="form-control select2">
                                        <option value=""> اختر مخزن الصرف </option>
                                        @if (@isset($stores) && !@empty($stores))
                                            @foreach ($stores as $info)
                                                <option @if (old('transfer_from_store_id', $data['transfer_from_store_id']) == $info->id) selected="selected" @endif
                                ...
                                <div class="form-group">
                                    ...
                                    <select @if ($added_counter_details > 0) disabled @endif name="transfer_to_store_id"
                                        id="transfer_to_store_id" class="form-control select2">
                                        <option value=""> اختر مخزن الاستلام </option>
                                        @if (@isset($stores) && !@empty($stores))
                                            @foreach ($stores as $info)
                                                <option @if (old('transfer_to_store_id', $data['transfer_to_store_id']) == $info->id) selected="selected" @endif
                                                    value="{{ $info->id }}"> {{ $info->name }} </option>
                                            @endforeach
                                        @endif
                                    </select>
