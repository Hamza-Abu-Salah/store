                                ...
                                <tbody>
                                    @foreach ($data as $info)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($info->inventory_type == 1)
                                                    جرد يومي
                                                @elseif($info->inventory_type == 2)
                                                    جرد اسبوعي
                                                @elseif($info->inventory_type == 3)
                                                    جرد شهري
                                                @elseif($info->inventory_type == 4)
                                                    جرد سنوي
                                                @else
                                                    لم يحدد
                                                @endif
                                            ...
                                            <td>
                                                @if ($info->is_closed == 0)
                                                    <a href="{{ route('admin.stores_inventory.edit', $info->id) }}"
                                                        class="btn btn-sm  btn-success">تعديل</a>
                                                    <a href="{{ route('admin.stores_inventory.delete', $info->id) }}"
                                                        class="btn btn-sm are_you_shue  btn-danger">حذف</a>
                                                @endif
                                                <a href="{{ route('admin.stores_inventory.show', $info->id) }}"
                                                    class="btn btn-sm   btn-info">التفاصيل</a>
                                                <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white"
                                                    target="_blank"
                                                    href="{{ route('admin.stores_inventory.printsaleswina4', [$info->id, 'A4']) }}"
