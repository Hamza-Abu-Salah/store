                ...
                <div class="card-body">
                            ...
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                        @if ($data['is_closed'] == 0)
                                            <a href="{{ route('admin.stores_inventory.delete', $data['id']) }}"
                                                class="btn btn-sm are_you_shue  btn-danger">حذف</a>
                                            <a href="{{ route('admin.stores_inventory.edit', $data['id']) }}"
                                                class="btn btn-sm btn-success">تعديل</a>
                                            <a href="{{ route('admin.stores_inventory.do_close_parent', $data['id']) }}"
                                                class="btn btn-sm are_you_shue  btn-primary">اغلاق وترحيل أمر الجرد</a>
                                        @endif
                                    </td>
                                </tr>
                            </table>
                    ...
                    <div class="card-header">
                        <h3 class="card-title card_title_center">
                            ...
                            @if ($data['is_closed'] == 0)
                                <button type="button" class="btn btn-info" data-target="#AddDetailsModal"
                                    data-toggle="modal">
                        ...
                        <input type="hidden" id="ajax_load_edit_item_details"
                            value="{{ route('admin.stores_inventory.load_edit_item_details') }}">
                                            ...
                                            @if ($data['is_closed'] == 0)
                                                <td>
                                                    @if ($info->is_closed == 0)
                                                        ...
                                                        <a href="{{ route('admin.stores_inventory.close_one_details', ['id' => $info->id, 'id_parent' => $data['id']]) }}"
                                                            class="btn btn-sm are_you_shue  btn-success">ترحيل</a>
                                                        <a href="{{ route('admin.stores_inventory.delete_details', ['id' => $info->id, 'id_parent' => $data['id']]) }}"
                                                            class="btn btn-sm are_you_shue  btn-danger">حذف</a>
                                                    @else
                                                        مغلق ومرحل
                                                    @endif
                ...
                <div class="modal-body" id="Add_item_Modal_body" style="background-color: white !important; color:black;">
                    <form action="{{ route('admin.stores_inventory.add_new_details', $data['id']) }}" method="post">
                        ...
                        <div class="form-group" id="ItemsDiv" style="display: none;">
                            ...
                            <select style="color:black" name="items_in_store" id="items_in_store"
                        ...
                        <div class="form-group text-center">
                            <button type="submit" name="submit" id="do_add_itmes" class="btn btn-sm btn-success">أضف للجرد
                            </button>
                        </div>
                </div>
...
@section('script')
    ...
    <script src="{{ asset('adminassets/js/inv_stores_inventory.js') }}"></script>
@endsection
