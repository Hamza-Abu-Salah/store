...
use App\Http\Requests\ItemCard1Request;
...
class Inv_itemCardController extends Controller
{
    public function index()
    {
        ...
        $data = get_cols_where_p(new Inv_itemCard(), array("*"), array("come_code" => $come_code), 'id', 'DESC', 10);
        if (!empty($data)) {
        ...
        $inv_itemcard_categories = get_cols_where(new Inv_itemCard_category(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
        return view('admin.inv_itemCards.index', ['data' => $data, 'inv_itemcard_categories' => $inv_itemcard_categories]);
    }
    public function create()
    {
        $come_code = auth()->user()->come_code;
        $inv_itemcard_categories = get_cols_where(new Inv_itemCard_category(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
        $inv_uoms_parent = get_cols_where(new Inv_uom(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1, 'is_master' => 1), 'id', 'DESC');
        $inv_uoms_child = get_cols_where(new Inv_uom(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1, 'is_master' => 0), 'id', 'DESC');
        $item_card_data = get_cols_where(new Inv_itemCard(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
        return view('admin.inv_itemCards.create', ['inv_itemcard_categories' => $inv_itemcard_categories, 'inv_uoms_parent' => $inv_uoms_parent, 'inv_uoms_child' => $inv_uoms_child, 'item_card_data' => $item_card_data]);
    }
    public function store(ItemcardRequest $request)
    {
        try {
            ...
            $row = get_cols_where_row_orderby(new Inv_itemCard(), array("item_code"), array("come_code" => $come_code), 'id', 'DESC');
            if (!empty($row)) {
            ...
            if ($request->barcode != '') {
                $checkExists_barcode = Inv_itemCard::where(['barcode' => $request->barcode, 'come_code' => $come_code])->first();
            ...
            $checkExists_barcode = Inv_itemCard::where(['name' => $request->name, 'come_code' => $come_code])->first();
            if (!empty($checkExists_barcode)) {
                return redirect()->back()
            ...
            if ($request->has('Item_img')) {
                ...
                $the_file_path = uploadImage('adminassets/uploads', $request->Item_img);
    ...
    public function edit($id)
    {
        ...
        $inv_uoms_child = get_cols_where(new Inv_uom(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1, 'is_master' => 0), 'id', 'DESC');
        $item_card_data = get_cols_where(new Inv_itemCard(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
        $counterUsedin_with_suppliers = get_count_where(new Supplier_with_order_detail(), array("come_code" => $come_code, "item_code" => $data['item_code']));
        $counterUsedin_with_sales = get_count_where(new Sales_invoices_detail(), array("come_code" => $come_code, "item_code" => $data['item_code']));
        $counterUsedBefore = $counterUsedin_with_suppliers + $counterUsedin_with_sales;
        return view('admin.inv_itemCards.edit', ['data' => $data, 'inv_itemcard_categories' => $inv_itemcard_categories, 'inv_uoms_parent' => $inv_uoms_parent, 'inv_uoms_child' => $inv_uoms_child, 'item_card_data' => $item_card_data, 'counterUsedBefore' => $counterUsedBefore]);
    }
    public function update($id, ItemcardRequestUpdate $request)
    {
        try {
            ...
            //check if not exsits for barcode
            if ($request->barcode != '') {
                $checkExists_barcode = Inv_itemCard::where(['barcode' => $request->barcode, 'come_code' => $come_code])->where("id", "!=", $id)->first();
                if (!empty($checkExists_barcode)) {
                    return redirect()->back()
                        ->with(['error' => 'عفوا باركود الصنف مسجل من قبل'])
                        ->withInput();
            ...
            //check if not exsits for name
            $checkExists_barcode = Inv_itemCard::where(['name' => $request->name, 'come_code' => $come_code])->where("id", "!=", $id)->first();
            if (!empty($checkExists_barcode)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا اسم الصنف مسجل من قبل'])
                    ->withInput();
            ...
            update(new Inv_itemCard(), $data_to_update, array('id' => $id, 'come_code' => $come_code));
    ...
    public function delete($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $item_row = get_cols_where_row(new Inv_itemCard(), array("id"), array("id" => $id, 'come_code' => $come_code));
            if (!empty($item_row)) {
                $flag = delete(new Inv_itemCard(), array("id" => $id, 'come_code' => $come_code));
                if ($flag) {
                    return redirect()->back()
    ...
    public function show($id)
    {
        ...
        $stores = get_cols_where(new Store(), array("id", "name"), array("come_code" => $come_code), "id", "ASC");
        return view('admin.inv_itemCards.show', ['data' => $data, 'inv_itemcard_movements_categories' => $inv_itemcard_movements_categories, 'inv_itemcard_movements_types' => $inv_itemcard_movements_types, 'stores' => $stores]);
    }
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            return view('admin.inv_itemCards.ajax_search', ['data' => $data]);
        }
    }
    public function ajax_search_movements(Request $request)
    {
        if ($request->ajax()) {
            ...
            return view('admin.inv_itemCards.ajax_search_movements', ['data' => $data]);
        }
    }
}
