...
class Inv_production_exchangeController extends Controller
{
    public function index()
    {
        ...
        $data = get_cols_where_p(new Inv_production_exchange(), array("*"), array("come_code" => $come_code, 'order_type' => 1), 'id', 'DESC', 10);
        if (!empty($data)) {
        ...
        $Inv_production_lines = get_cols_where(new Inv_production_lines(), array('production_lines_code', 'name'), array('come_code' => $come_code), 'id', 'ASC');
    ...
    {
        $come_code = auth()->user()->come_code;
        $Inv_production_lines = get_cols_where(new Inv_production_lines(), array('production_lines_code', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'ASC');
        $stores = get_cols_where(new Store(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
        $Inv_production_order = get_cols_where(new Inv_production_order(), array('auto_serial'), array('come_code' => $come_code, 'is_closed' => 0, 'is_approved' => 1), 'id', 'DESC');
    ...
    public function store(Inv_production_exchangeRequest $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            $Inv_production_order_data = get_cols_where_row(new Inv_production_order(), array("is_approved", "is_closed"), array("auto_serial" => $request->inv_production_order_auto_serial, "come_code" => $come_code));
            ...
            $Inv_production_line_data = get_cols_where_row(new Inv_production_lines(), array("account_number"), array("production_lines_code" => $request->production_lines_code, "come_code" => $come_code));
            if (empty($Inv_production_line_data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا   غير قادر علي الوصول الي بيانات خط الانتاج  المحدد'])
                    ->withInput();
            }
            $row = get_cols_where_row_orderby(new Inv_production_exchange(), array("auto_serial"), array("come_code" => $come_code, 'order_type' => 1), 'id', 'DESC');
            if (!empty($row)) {
            ...
            //$id = get_field_value(new Suppliers_with_orders(), "id", array("auto_serial" => $data_insert['auto_serial'], "come_code" => $come_code, "order_type" => 3));
    ...
    {
        $come_code = auth()->user()->come_code;
        $data = get_cols_where_row(new Inv_production_exchange(), array("*"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
        if (empty($data)) {
        ...
        $Inv_production_order = get_cols_where(new Inv_production_order(), array('auto_serial'), array('come_code' => $come_code, 'is_closed' => 0, 'is_approved' => 1), 'id', 'DESC');
        $added_counter_details = get_count_where(new Inv_production_exchange_details(), array("come_code" => $come_code, "order_type" => 1, "inv_production_exchange_auto_serial" => $data['auto_serial']));
    ...
    public function update($id, inv_production_exchangeUpRequest $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Inv_production_exchange(), array("is_approved", "auto_serial"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($data)) {
                return redirect()->route('admin.inv_production_exchange.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            if ($data['is_approved'] == 1) {
            ...
            $data_Inv_production_line = get_cols_where_row(new Inv_production_lines(), array("account_number"), array("production_lines_code" => $request->production_lines_code, "come_code" => $come_code));
            ...
            update(new Inv_production_exchange(), $data_to_update, array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
    ...
    public function show($id)
    {
        try {
            ...
            $details = get_cols_where(new Inv_production_exchange_details(), array("*"), array('inv_production_exchange_auto_serial' => $data['auto_serial'], 'order_type' => 1, 'come_code' => $come_code), 'id', 'DESC');
            if (!empty($details)) {
    ...
    public function load_modal_add_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new Inv_production_exchange(), array("is_approved", "store_id"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
    ...
    public function Add_item_to_invoice(Request $request)
    {
        try {
            if ($request->ajax()) {
                $come_code = auth()->user()->come_code;
                $invoice_data = get_cols_where_row(new Inv_production_exchange(), array("is_approved", "order_date", "production_lines_code"), array("come_code" => $come_code, "auto_serial" => $request->autoserailparent, 'order_type' => 1));
    ...
    {
        $come_code = auth()->user()->come_code;
        $invoice_data = get_cols_where_row(new Inv_production_exchange(), array("*"), array("come_code" => $come_code, "auto_serial" => $auto_serial, 'order_type' => 1));
        if (!empty($invoice_data)) {
            //first get sum of details
            $dataUpdateParent['total_cost_items'] = get_sum_where(new Inv_production_exchange_details(), "total_price", array("come_code" => $come_code, "inv_production_exchange_auto_serial" => $auto_serial, 'order_type' => 1));
            ...
            update(new Inv_production_exchange(), $dataUpdateParent, array("come_code" => $come_code, "auto_serial" => $auto_serial, 'order_type' => 1));
        }
    }
    ...
    public function reload_parent_pill(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Inv_production_exchange(), array("*"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($data)) {
                $data['added_by_admin'] = User::where('id', $data['added_by'])->value('name');
                $data['production_lines_name'] = Inv_production_lines::where('production_lines_code', $data['production_lines_code'])->value('name');
                $data['store_name'] = Store::where('id', $data['store_id'])->value('name');
    ...
    public function reload_itemsdetials(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = get_cols_where_row(new Inv_production_exchange(), array("is_approved", "id"), array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));

            $details = get_cols_where(new Inv_production_exchange_details(), array("*"), array('inv_production_exchange_auto_serial' => $auto_serial, 'order_type' => 1, 'come_code' => $come_code), 'id', 'DESC');
            if (!empty($details)) {
                foreach ($details as $info) {
                    $info->item_card_name = Inv_itemCard::where('item_code', $info->item_code)->value('name');
                    $info->uom_name = get_field_value(new Inv_uom(), "name", array("id" => $info->uom_id));
    ...
    public function delete_details($id, $parent_id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new Inv_production_exchange(), array("is_approved", "auto_serial", "store_id", "production_lines_code"), array("id" => $parent_id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($parent_pill_data)) {
                return redirect()->back()
                    ...
                    if (!empty($itemCard_Data) and !empty($batch_data)) {
                        ...
                        if ($flag) {
                            $quantityAfterMove = get_sum_where(
                                new Inv_itemcard_batches(),
                                "quantity",
                                array("item_code" => $item_row['item_code'], "come_code" => $come_code)
                            );
                            //get Quantity Befor any Action  حنجيب كيمة الصنف  بالمخزن المحدد معه   الحالي بعد الحركة
                            $quantityAfterMoveCurrentStore = get_sum_where(
                                new Inv_itemcard_batches(),
    ...
    public function delete($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new Inv_production_exchange(), array("is_approved", "auto_serial", "store_id", "production_lines_code"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($parent_pill_data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا حدث خطأ ما']);
            }
            ...
            $items_details = get_cols_where(new Inv_production_exchange_details(), array("*"), array("come_code" => $come_code, "order_type" => 1, "inv_production_exchange_auto_serial" => $parent_pill_data['auto_serial']));
            //حنحذف الفاتورة الاب
            $flag = delete(new Inv_production_exchange(), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if ($flag) {
                ...
                if (!empty($items_details)) {
                    foreach ($items_details as $info) {
                        $flagDelete = delete(new Inv_production_exchange_details(), array("come_code" => $come_code, "order_type" => 1, "inv_production_exchange_auto_serial" => $parent_pill_data['auto_serial'], 'id' => $info->id));
                        if ($flagDelete) {
    ...
    public function load_modal_approve_invoice(Request $request)
    {
        if ($request->ajax()) {
            ...
            $counterDetails = get_count_where(new Inv_production_exchange_details(), array("inv_production_exchange_auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
    ...
    function do_approve($auto_serial, Request $request)
    {
        ...
        $data = get_cols_where_row(new Inv_production_exchange(), array("total_cost_items", "is_approved", "id", "account_number", "store_id", "production_lines_code"), array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
        if (empty($data)) {
            return redirect()->route("admin.inv_production_exchange.index")->with(['error' => "عفوا غير قادر علي الوصول الي البيانات المطلوبة !!"]);
        }
        if ($data['is_approved'] == 1) {
        ...
        $counterDetails = get_count_where(new Inv_production_exchange_details(), array("inv_production_exchange_auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
        if ($counterDetails == 0) {
        ...
        $flag = update(new Inv_production_exchange(), $dataUpdateParent, array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
        if ($flag) {
...
$last_record_treasuries_transactions_record = get_cols_where_row_orderby(new Treasuries_transactions(), array("auto_serial"), array("come_code" => $come_code), "auto_serial", "DESC");
...
*/
    ...
    public function printsaleswina4($id, $size)
    {
        ...
        try {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new Inv_production_exchange(), array("*"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($invoice_data)) {
                return redirect()->route('admin.inv_production_exchange.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            ...
            $invoices_details = get_cols_where(new Inv_production_exchange_details(), array("*"), array('inv_production_exchange_auto_serial' => $invoice_data['auto_serial'], 'order_type' => 1, 'come_code' => $come_code), 'id', 'ASC');
