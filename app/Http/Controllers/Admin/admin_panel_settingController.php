...
class admin_panel_settingController extends Controller
{
    ...
    {
        $data = Admin_panel_setting::where('come_code', auth()->user()->come_code)->first();
        if (!empty($data)) {
        ...
        return view('admin.admin-panel-setting.index', ['data' => $data]);
    }
    public function edit()
    {
        $data = Admin_panel_setting::where('come_code', auth()->user()->come_code)->first();
        $parent_accounts = get_cols_where(new Account(), array("account_number", "name"), array("is_parent" => 1, "come_code" => auth()->user()->come_code), 'id', 'ASC');
        return view('admin.admin-panel-setting.edit', ['data' => $data, 'parent_accounts' => $parent_accounts]);
    }
    public function update(adminRequest $request)
    {
        try {
            $admin_panel_setting = Admin_panel_setting::where('come_code', auth()->user()->come_code)->first();
            ...
            if ($request->has('photo')) {
                ...
                if (file_exists('uploads/' . $oldphotoPath) and !empty($oldphotoPath)) {
                    unlink('uploads/' . $oldphotoPath);
                }
            }
