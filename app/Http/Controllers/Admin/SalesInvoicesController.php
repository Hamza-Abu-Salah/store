...
class SalesInvoicesController extends Controller
{
    public function index()
    {
        ...
        $data = get_cols_where_p(new Sales_invoice(), array("*"), array("come_code" => $come_code), "id", "DESC", 10);
        if (!empty($data)) {
            foreach ($data as $info) {
                $info->added_by_admin = User::where('id', $info->added_by)->value('name');
                $info->Sales_matrial_types_name = get_field_value(new Sales_matrail_type(), "name", array("come_code" => $come_code, "id" => $info->sales_matrial_types));
                if ($info->is_has_customer == 1) {
                    $info->customer_name = get_field_value(new Customer(), "name", array("come_code" => $come_code, "customer_code" => $info->customer_code));
                } else {
                    $info->customer_name = "بدون عميل";
                }
            }
        }
        $delegates = get_cols_where(new Delegate(), array("delegate_code", "name"), array("come_code" => $come_code, "active" => 1));
        $customers = get_cols_where(new Customer(), array("customer_code", "name"), array("come_code" => $come_code, "active" => 1));
        $Sales_matrial_types = get_cols_where(new Sales_matrail_type(), array("id", "name"), array("come_code" => $come_code, "active" => 1));
    ...
    public function load_modal_addMirror(Request $request)
    {
        $come_code = auth()->user()->come_code;
        if ($request->ajax()) {
            $item_cards = get_cols_where(new Inv_itemCard(), array("item_code", "name", "item_type"), array("come_code" => $come_code, "active" => 1));
            $stores = get_cols_where(new Store(), array("id", "name"), array("come_code" => $come_code, "active" => 1), 'id', 'ASC');
            $user_shift = get_user_shift(new Admin_Shift(), new Treasuries(), new Treasuries_transaction());
    ...
    public function load_modal_addActiveInvoice(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return view("admin.sales_invoices.load_modal_addActiveInvoice", compact('Sales_matrial_types', 'delegates'));
        }
    }
        ...
        if ($request->ajax()) {
            ...
            if (!empty($item_card_Data)) {
                ...
                if (!empty($uom_Data)) {
                    ...
                    return view("admin.sales_invoices.get_items_batches", ['item_card_Data' => $item_card_Data, 'requesed' => $requesed, 'uom_Data' => $uom_Data, 'inv_itemcard_batches' => $inv_itemcard_batches]);
                }
            }
        }
    }
    public function get_item_unit_price(Request $request)
    {
        $come_code = auth()->user()->come_code;
        if ($request->ajax()) {
            $item_card_Data = get_cols_where_row(new Inv_itemCard(), array("uom_id", "price", "nos_gomla_price", "gomla_price", "price_retail", "nos_gomla_price_retail", "gomla_price_retail", "does_has_retailunit", "retail_uom_id"), array("come_code" => $come_code, "item_code" => $request->item_code));
            if (!empty($item_card_Data)) {
                $uom_id = $request->uom_id;
                $sales_item_type = $request->sales_item_type;
                $uom_Data = get_cols_where_row(new Inv_uom(), array("is_master"), array("come_code" => $come_code, "id" => $uom_id));
                if (!empty($uom_Data)) {
    ...
    public function store(Request $request)
    {
        if ($request->ajax()) {
            ...
            $last_auto_serial_Date = get_cols_where_row_orderby(new Sales_invoice(), array("auto_serial"), array("come_code" => $come_code), 'id', 'DESC');
            ...
            $flag = insert(new Sales_invoice(), $data_insert, false);
            if ($flag) {
    ...
    public function load_invoice_update_modal(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $invoice_data = Sales_invoice::select("*")->where(["come_code" => $come_code, "auto_serial" => $request->auto_serial])->first();
            $stores = Store::select("id", "name")->where(["come_code" => $come_code, "active" => 1])->orderby('id', 'ASC')->get();
            $user_shift = get_user_shift(new Admin_Shift(), new Treasuries(), new Treasuries_transaction());
            $delegates = Delegate::select("delegate_code", "name")->where(["come_code" => $come_code, "active" => 1])->get();
            $customers = Customer::select("customer_code", "name")->where(["come_code" => $come_code, "customer_code" => $invoice_data['customer_code']])->get();
            $Sales_matrial_types = Sales_matrail_type::select("id", "name")->where(["come_code" => $come_code, "active" => 1])->get();
            $sales_invoices_details = Sales_invoices_detail::select()->where(["come_code" => $come_code, "sales_invoices_auto_serial" => $request->auto_serial])->get();
            if (!empty($sales_invoices_details)) {
                foreach ($sales_invoices_details  as $info) {
                    $info->store_name = Store::where(["come_code" => $come_code, "id" => $info->store_id])->value("name");
                    $info->item_name = Inv_itemCard::where(["come_code" => $come_code, "item_code" => $info->item_code])->value("name");
                    $info->uom_name = Inv_uom::where(["come_code" => $come_code, "id" => $info->uom_id])->value("name");
                }
            }
            return view("admin.sales_invoices.load_invoice_update_modal", compact('stores', 'user_shift', 'delegates', 'customers', 'Sales_matrial_types', 'invoice_data', 'sales_invoices_details'));
        }
    }
    public function Add_item_to_invoice(Request $request)
    {
        try {
            if ($request->ajax()) {
                $come_code = auth()->user()->come_code;
                $invoice_data = get_cols_where_row(new Sales_invoice(), array("is_approved", "invoice_date", "is_has_customer", "customer_code"), array("come_code" => $come_code, "auto_serial" => $request->invoiceautoserial));
                if (!empty($invoice_data)) {
                    if ($invoice_data['is_approved'] == 0) {
                        $batch_data = get_cols_where_row(new Inv_itemcard_batches(), array("quantity", "unit_cost_price", "id"), array("come_code" => $come_code, "auto_serial" => $request->inv_itemcard_batches_autoserial, 'store_id' => $request->store_id, 'item_code' => $request->item_code));
                        if (!empty($batch_data)) {
                            if ($batch_data['quantity'] >= $request->item_quantity) {
                                $itemCard_Data = get_cols_where_row(new Inv_itemCard(), array("uom_id", "retail_uom_quntToParent", "retail_uom_id", "does_has_retailunit"), array("come_code" => $come_code, "item_code" => $request->item_code));
                                if (!empty($itemCard_Data)) {
                                    ...
                                    $flag_datainsert_items = insert(new Sales_invoices_detail(), $datainsert_items, true);
                                    if (!empty($flag_datainsert_items)) {
                                        ...
                                        if ($flag) {
                                            ...
                                            if ($invoice_data['is_has_customer'] == 1) {
                                                $customerName = get_field_value(new Customer(), "name", array("come_code" => $come_code, "customer_code" => $invoice_data['customer_code']));
                                            } else {
    ...
    function reload_items_in_invoice(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $sales_invoices_details = get_cols_where(new Sales_invoices_detail(), array("*"), array("come_code" => $come_code, "sales_invoices_auto_serial" => $request->auto_serial));
            if (!empty($sales_invoices_details)) {
                foreach ($sales_invoices_details  as $info) {
                    $info->store_name = get_field_value(new Store(), "name", array("come_code" => $come_code, "id" => $info->store_id));
                    $info->item_name = get_field_value(new Inv_itemCard(), "name", array("come_code" => $come_code, "item_code" => $info->item_code));
                    $info->uom_name = get_field_value(new Inv_uom(), "name", array("come_code" => $come_code, "id" => $info->uom_id));
                }
            }
    ...
    function recalclate_parent_invoice(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new Sales_invoice(), array("*"), array("come_code" => $come_code, "auto_serial" => $request->auto_serial));
            if (!empty($invoice_data)) {
                if ($invoice_data['is_approved'] == 0) {
                    ...
                    update(new Sales_invoice(), $dataUpdateParent, array("come_code" => $come_code, "auto_serial" => $request->auto_serial));
                    echo json_encode("done");
                }
            }
        }
    ...
    function remove_active_row_item(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new Sales_invoice(), array("is_approved", "is_has_customer", "customer_code"), array("come_code" => $come_code, "auto_serial" => $request->auto_serial));
            if (!empty($invoice_data)) {
                if ($invoice_data['is_approved'] == 0) {
                    $sales_invoices_details_data = get_cols_where_row(new Sales_invoices_detail(), array("batch_auto_serial", "quantity", "item_code", "store_id", "isparentuom"), array("come_code" => $come_code, "id" => $request->id));
                    if (!empty($sales_invoices_details_data)) {
                        $batch_data = get_cols_where_row(new Inv_itemcard_batches(), array("quantity", "unit_cost_price", "id"), array("come_code" => $come_code, "auto_serial" => $sales_invoices_details_data['batch_auto_serial']));
                        if (!empty($batch_data)) {
                            $itemCard_Data = get_cols_where_row(new Inv_itemCard(), array("uom_id", "retail_uom_quntToParent", "retail_uom_id", "does_has_retailunit"), array("come_code" => $come_code, "item_code" => $sales_invoices_details_data['item_code']));
                            if (!empty($itemCard_Data)) {
                                $MainUomName = get_field_value(new Inv_uom(), "name", array("come_code" => $come_code, "id" => $itemCard_Data['uom_id']));
                                //حذف السطر من تفاصيل الفاتورة
                                $flag = delete(new Sales_invoices_detail(), array("come_code" => $come_code, "id" => $request->id));
                                if ($flag) {
                                    ...
                                    $quantityBeforMoveCurrntStore = get_sum_where(
                                        ...
                                        "quantity",
                                        array(
                                            "item_code" => $sales_invoices_details_data['item_code'], "come_code" => $come_code,
                                    ...
                                    if ($flag) {
                                        ...
                                        $quantityAfterMoveCurrentStore = get_sum_where(
                                            new Inv_itemcard_batches(),
                                            "quantity",
                                            array("item_code" => $sales_invoices_details_data['item_code'], "come_code" => $come_code, 'store_id' => $sales_invoices_details_data['store_id'])
                                        );
                                        //التاثير في حركة كارت الصنف
                                        $dataInsert_inv_itemcard_movements['inv_itemcard_movements_categories'] = 2;
    ...
    function DoApproveInvoiceFinally(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new Sales_invoice(), array("is_approved", "pill_type", "total_cost", "customer_code", "is_has_customer", "delegate_code", "sales_item_type"), array("come_code" => $come_code, "auto_serial" => $request->auto_serial));
            if (!empty($invoice_data)) {
                if ($invoice_data['is_approved'] == 0) {
                    $dataUpdateParent['money_for_account'] = $invoice_data['total_cost'];
                    $dataUpdateParent['is_approved'] = 1;
                    $dataUpdateParent['approved_by'] = auth()->user()->come_code;
                    $dataUpdateParent['updated_at'] = date("Y-m-d H:i:s");
                    $dataUpdateParent['updated_by'] = auth()->user()->come_code;
                    $dataUpdateParent['what_paid'] = $request->what_paid;
                    $dataUpdateParent['what_remain'] = $request->what_remain;
                    if ($invoice_data['is_has_customer'] == 1) {
                        $customerData = get_cols_where_row(new Customer(), array("account_number"), array("come_code" => $come_code, "customer_code" => $invoice_data['customer_code']));
                    ...
                    if ($invoice_data['delegate_code'] > 0 and $invoice_data['delegate_code'] != null) {
                        $delegateData = get_cols_where_row(new Delegate(), array("percent_type", "percent_salaes_commission_kataei", "percent_salaes_commission_nosjomla", "percent_salaes_commission_jomla"), array("come_code" => $come_code, "delegate_code" => $invoice_data['delegate_code']));
                    ...
                    $flag = update(new Sales_invoice(), $dataUpdateParent, array("come_code" => $come_code, "auto_serial" => $request->auto_serial));
                    if ($flag) {
                        $DelegateData = get_cols_where_row(new Delegate(), array("account_number"), array("come_code" => $come_code, "delegate_code" => $invoice_data['delegate_code']));
                        if (!empty($DelegateData)) {
                            refresh_account_blance_delegate($DelegateData['account_number'], new Account(), new Delegate(), new Treasuries_transaction(), new Sales_invoice(), new Services_with_orders(), false);
                        }
                        if ($request->what_paid > 0) {
                            $user_shift = get_user_shift(new Admin_Shift(), new Treasuries(), new Treasuries_transaction());
                            $treasury_date = get_cols_where_row(new Treasuries(), array("last_isal_collect"), array("come_code" => $come_code, "id" => $user_shift['treasuries_id']));
                            ...
                            $dataInsert_treasuries_transactions['come_code'] = $come_code;
                            $flag = insert(new Treasuries_transaction(), $dataInsert_treasuries_transactions);
                            if ($flag) {
                                ...
                                update(new Treasuries(), $dataUpdateTreasuries, array("come_code" => $come_code, "id" => $user_shift['treasuries_id']));
                            }
                        }
                        if ($invoice_data['is_has_customer'] == 1) {
                            //Affect on Customer Finanical Account Balance
                            refresh_account_blance_customer($customerData["account_number"], new Account(), new Customer(), new Treasuries_transaction(), new Sales_invoice(), new SalesReturn(), new services_with_orders(), false);
                        }
                        echo json_encode("done");
                    }
                }
    ...
    public function delete($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new Sales_invoice(), array("*"), array("id" => $id, "come_code" => $come_code));
            if (empty($invoice_data)) {
            ...
            $sales_invoices_details = get_cols_where(new Sales_invoices_detail(), array("*"), array("come_code" => $come_code, "sales_invoices_auto_serial" => $invoice_data['auto_serial']));

            if (!empty($sales_invoices_details)) {

                foreach ($sales_invoices_details as $item) {

                    $batch_data = get_cols_where_row(new Inv_itemcard_batches(), array("quantity", "unit_cost_price", "id"), array("come_code" => $come_code, "auto_serial" => $item->batch_auto_serial));
                    if (!empty($batch_data)) {
                        $itemCard_Data = get_cols_where_row(new Inv_itemCard(), array("uom_id", "retail_uom_quntToParent", "retail_uom_id", "does_has_retailunit"), array("come_code" => $come_code, "item_code" => $item->item_code));
                        if (!empty($itemCard_Data)) {
                            $MainUomName = get_field_value(new Inv_uom(), "name", array("come_code" => $come_code, "id" => $itemCard_Data['uom_id']));
                            //حذف السطر من تفاصيل الفاتورة
                            $flag = delete(new Sales_invoices_detail(), array("come_code" => $come_code, "id" => $item->id));
                            if ($flag) {
                                //رد الكمية الي الباتش
                                //كمية الصنف بكل المخازن قبل الحركة
                                $quantityBeforMove = get_sum_where(
                                ...
                                $quantityBeforMoveCurrntStore = get_sum_where(
                                    ...
                                    "quantity",
                                    array(
                                        "item_code" => $item->item_code, "come_code" => $come_code,
                                ...
                                if ($flag) {
                                    ...
                                    $quantityAfterMoveCurrentStore = get_sum_where(
                                        new Inv_itemcard_batches(),
                                        "quantity",
                                        array("item_code" => $item->item_code, "come_code" => $come_code, 'store_id' => $item->store_id)
                                    );
                                    //التاثير في حركة كارت الصنف
                                    $dataInsert_inv_itemcard_movements['inv_itemcard_movements_categories'] = 2;
                                    $dataInsert_inv_itemcard_movements['items_movements_types'] = 15;
            ...
            $flag = delete(new Sales_invoice(), array("id" => $id, "come_code" => $come_code));
            if ($flag) {
                return redirect()->back()
    ...
    public function load_invoice_details_modal(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new Sales_invoice(), array("*"), array("come_code" => $come_code, "auto_serial" => $request->auto_serial));
            $delegates = get_cols_where(new Delegate(), array("delegate_code", "name"), array("come_code" => $come_code, "delegate_code" => $invoice_data['delegate_code']));
            if ($invoice_data['is_has_customer'] == 1) {
                $customers = get_cols_where(new Customer(), array("customer_code", "name"), array("come_code" => $come_code, "customer_code" => $invoice_data['customer_code']));
            } else {
                $customers = array();
            }

            $Sales_matrial_types = get_cols_where(new Sales_matrail_type(), array("id", "name"), array("come_code" => $come_code, "id" => $invoice_data['sales_matrial_types']));
            $sales_invoices_details = get_cols_where(new Sales_invoices_detail(), array("*"), array("come_code" => $come_code, "sales_invoices_auto_serial" => $request->auto_serial));
            if (!empty($sales_invoices_details)) {
                foreach ($sales_invoices_details  as $info) {
                    $info->store_name = get_field_value(new Store(), "name", array("come_code" => $come_code, "id" => $info->store_id));
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Sales_invoice::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where($field4, $operator4, $value4)->where($field5, $operator5, $value5)->where($field6, $operator6, $value6)
    ...
    public function get_last_added_customer(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $customers = get_cols_where_limit(new Customer(), array("customer_code", "name"), array("come_code" => $come_code), 'id', 'DESC', 1);
    ...
    {
        $come_code = auth()->user()->come_code;
        $invoice_data = get_cols_where_row(new Sales_invoice(), array("*"), array("come_code" => $come_code, "id" => $id));
        if (empty($invoice_data)) {
            return redirect()->back()->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة']);
        }
        $invoice_data['customer_name'] = get_field_value(new Customer(), 'name', array("come_code" => $come_code, "customer_code" => $invoice_data['customer_code']));
        $invoice_data['customer_phones'] = get_field_value(new Customer(), 'phones', array("come_code" => $come_code, "customer_code" => $invoice_data['customer_code']));
