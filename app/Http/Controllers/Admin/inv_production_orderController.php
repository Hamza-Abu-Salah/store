...
class inv_production_orderController extends Controller
{
    public function index()
    {
        try {
            ...
            $data = get_cols_where_p(new Inv_production_order(), array("*"), array("come_code" => $come_code), 'id', 'DESC', 10);
            if (!empty($data)) {
                foreach ($data as $info) {
                    ...
                    if ($info->is_approved == 1) {
                        $info->approved_by_admin = User::where('id', $info->approved_by)->value('name');
                    }
                    if ($info->is_closed == 1) {
                        $info->closed_by_admin = User::where('id', $info->closed_by)->value('name');
                    }
                }
            }
    ...
    public function store(inv_production_orderRequest $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            $row = Inv_production_order::select('auto_serial')->where(['come_code' => $come_code])->orderby('id', 'DESC')->first();
            if (!empty($row)) {
            ...
            Inv_production_order::create($data);
            return redirect()->route('admin.inv_production_order.index')->with(['success' => 'تم اضافة البيانات بنجاح']);
        } catch (\Exception $ex) {
    ...
    {
        $come_code = auth()->user()->come_code;
        $data = get_cols_where_row(new Inv_production_order(), array("*"), array("id" => $id, "come_code" => $come_code));
        if (empty($data)) {
    ...
    public function update($id, inv_production_orderRequest $request)
    {
        try {
            ...
            update(new Inv_production_order(), $data_to_update, array("id" => $id, "come_code" => $come_code));
    ...
    public function delete($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Inv_production_order(), array("is_closed"), array("id" => $id, "come_code" => $come_code));
            if (empty($data)) {
                return redirect()->back()
            ...
            $flag = delete(new Inv_production_order(), array("id" => $id, "come_code" => $come_code));
            if ($flag) {
    ...
    public function show_more_detials(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new  Inv_production_order(), array("*"), array("id" => $request->id, "come_code" => $come_code));
            if (!empty($data)) {
    ...
    public function do_approve($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Inv_production_order(), array("is_approved"), array("id" => $id, "come_code" => $come_code));
            if (empty($data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا حدث خطأ ما']);
            }
            ...
            $flag = update(new Inv_production_order(), $data_to_update, array("id" => $id, "come_code" => $come_code));
    ...
    public function do_closes_archive($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Inv_production_order(), array("is_approved", "is_closed"), array("id" => $id, "come_code" => $come_code));
            if (empty($data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا حدث خطأ ما']);
            }
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = inv_production_order::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where($field4, $operator4, $value4)->where($field5, $operator5, $value5)->where('come_code', '=', $come_code)->orderBy('id', 'DESC')->paginate(10);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = User::where('id', $info->added_by)->value('name');
                    if ($info->updated_by > 0 and $info->updated_by != null) {
