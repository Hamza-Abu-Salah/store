...
use App\Http\Requests\Services_order_request;
...
class Services_with_ordersController extends Controller
{
    public function index()
    {
        ...
        $data = get_cols_where_p(new services_with_orders(), array("*"), array("come_code" => $come_code), 'id', 'DESC', 10);
        if (!empty($data)) {
        ...
        $accounts = get_cols_where(new Account(), array('account_number', 'name'), array('come_code' => $come_code, 'is_parent' => 0), 'id', 'DESC');
    ...
    public function store(Services_order_request $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            if ($request->is_account_number == 1) {
                $AccountData = get_cols_where_row(new Account(), array("id"), array("account_number" => $request->account_number, "come_code" => $come_code));
            ...
            if ($request->order_type == 1) {
                $row = get_cols_where_row_orderby(new services_with_orders(), array("auto_serial"), array("come_code" => $come_code, 'order_type' => 1), 'id', 'DESC');
            } else {
                $row = get_cols_where_row_orderby(new services_with_orders(), array("auto_serial"), array("come_code" => $come_code, 'order_type' => 2), 'id', 'DESC');
            }
            if (!empty($row)) {
            ...
            $id = get_field_value(new services_with_orders(), "id", array("auto_serial" => $data_insert['auto_serial'], "come_code" => $come_code, "order_type" => $request->order_type));
    ...
    public function show($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new services_with_orders(), array("*"), array("id" => $id, "come_code" => $come_code));
            if (empty($data)) {
            ...
            $details = get_cols_where(new services_with_orders_details(), array("*"), array('services_with_orders_auto_serial' => $data['auto_serial'], 'order_type' => $data['order_type'], 'come_code' => $come_code), 'id', 'DESC');
            if (!empty($details)) {
    ...
    public function load_modal_add_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new services_with_orders(), array("is_approved", "order_type"), array("id" => $request->id_parent_pill, "come_code" => $come_code));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    $services = get_cols_where(new Services(), array("name", "id"), array('active' => 1, 'come_code' => $come_code, 'type' => $parent_pill_data['order_type']), 'id', 'DESC');
    ...
    public function add_new_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new services_with_orders(), array("is_approved", "order_date", "tax_value", "discount_value", 'order_type', 'auto_serial'), array("id" => $request->id_parent_pill, "come_code" => $come_code));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    ...
                    if ($flag) {
                        /** update parent pill */
                        $total_detials_sum = get_sum_where(new services_with_orders_details(), 'total', array("services_with_orders_auto_serial" => $parent_pill_data['auto_serial'], 'order_type' => $parent_pill_data['order_type'], 'come_code' => $come_code));
                        ...
                        update(new services_with_orders(), $dataUpdateParent, array("come_code" => $come_code, "id" => $request->id_parent_pill));
    ...
    public function delete($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new services_with_orders(), array("is_approved", "auto_serial", 'order_type'), array("id" => $id, "come_code" => $come_code));
            if (empty($parent_pill_data)) {
                return redirect()->back()
            ...
            $flag = delete(new services_with_orders(), array("id" => $id, "come_code" => $come_code));
            if ($flag) {
                delete(new services_with_orders_details(), array("services_with_orders_auto_serial" => $parent_pill_data['auto_serial'], "come_code" => $come_code, 'order_type' => $parent_pill_data['order_type']));
    ...
    public function edit($id)
    {
        ...
        $accounts = get_cols_where(new Account(), array('account_number', 'name'), array('come_code' => $come_code, 'is_parent' => 0), 'id', 'DESC');
        $Services_Added_Counter = get_count_where(new services_with_orders_details(), array('services_with_orders_auto_serial' => $data['auto_serial'], 'order_type' => $data['order_type'], 'come_code' => $come_code));

        return view('admin.services_with_orders.edit', ['data' => $data, 'accounts' => $accounts, 'Services_Added_Counter' => $Services_Added_Counter]);
    }


    public function update($id, Services_order_request $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new services_with_orders(), array("*"), array("id" => $id, "come_code" => $come_code));
            ...
            if ($request->is_account_number == 1) {
                $accountData = get_cols_where_row(new Account(), array("account_number"), array("account_number" => $request->account_number, "come_code" => $come_code));
                if (empty($accountData)) {
                    return redirect()->back()
            ...
            update(new services_with_orders(), $data_to_update, array("id" => $id, "come_code" => $come_code));
    ...
    public function reload_parent_pill(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new services_with_orders(), array("*"), array("id" => $request->id, "come_code" => $come_code));
            if (!empty($data)) {
                $data['added_by_admin'] = User::where('id', $data['added_by'])->value('name');
                if ($data['is_account_number'] == 1) {
                    $data['account_name'] = Account::where('account_number', $data['account_number'])->value('name');
    ...
    public function delete_details($id, $parent_id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new services_with_orders(), array("*"), array("id" => $parent_id, "come_code" => $come_code));
            if (empty($parent_pill_data)) {
                return redirect()->route('admin.Services_orders.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            if ($parent_pill_data['is_approved'] == 1) {
            ...
            if (!empty($item_row)) {
                ...
                if ($flag) {
                    ...
                    $dataUpdateParent['updated_by'] = auth()->user()->id;
                    $dataUpdateParent['updated_at'] = date("Y-m-d H:i:s");
                    update(new services_with_orders(), $dataUpdateParent, array("come_code" => $come_code, "id" => $parent_id));
    ...
    public function load_edit_item_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new services_with_orders(), array("*"), array("id" => $request->id_parent_pill, "come_code" => $come_code));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    $item_data_detials = get_cols_where_row(new services_with_orders_details(), array("*"), array("services_with_orders_auto_serial" => $parent_pill_data['auto_serial'], "come_code" => $come_code, 'order_type' => $parent_pill_data['order_type'], 'id' => $request->id));
                    $services = get_cols_where(new Services(), array("name", "id"), array('active' => 1, 'come_code' => $come_code, 'type' => $parent_pill_data['order_type']), 'id', 'DESC');
        ...
        if ($request->ajax()) {
            ...
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    ...
                    $flag = update(new services_with_orders_details(), $data_to_update, array("id" => $request->id, 'come_code' => $come_code, 'order_type' => $parent_pill_data['order_type'], 'services_with_orders_auto_serial' => $parent_pill_data['auto_serial']));
                    if ($flag) {
                        /** update parent pill */
    ...
    public function load_modal_approve_invoice(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new services_with_orders(), array("*"), array("id" => $request->id_parent_pill, "come_code" => $come_code));
            //current user shift
            $user_shift = get_user_shift(new Admin_Shift(), new Treasuries(), new Treasuries_transaction());
            $counterDetails = get_count_where(new services_with_orders_details(), array('services_with_orders_auto_serial' => $data['auto_serial'], 'order_type' => $data['order_type'], 'come_code' => $come_code));
    ...
    function do_approve($id, Request $request)
    {
        ...
        $data = get_cols_where_row(new services_with_orders(), array("total_services", "is_approved", "id", "account_number", "auto_serial", "order_type", "is_account_number", "entity_name"), array("id" => $id, "come_code" => $come_code));
        if (empty($data)) {
            return redirect()->route("admin.Services_orders.index")->with(['error' => "عفوا غير قادر علي الوصول الي البيانات المطلوبة !!"]);
        }
        if ($data['is_approved'] == 1) {
        ...
        $flag = update(new services_with_orders(), $dataUpdateParent, array("id" => $id, "come_code" => $come_code, 'is_approved' => 0));
        if ($flag) {
            ...
            if ($data['is_account_number'] == 1) {
                ...
                if ($account_type == 2) {
                    $the_final_Balance = refresh_account_blance_supplier($data['account_number'], new Account(), new Supplier(), new Treasuries_transaction(), new Supplier_with_order(), new services_with_orders(), false);
                } elseif ($account_type == 3) {
                    $the_final_Balance = refresh_account_blance_customer($data['account_number'], new Account(), new Customer(), new Treasuries_transaction(), new Sales_invoice(), new SalesReturn(), new services_with_orders(), false);
                } elseif ($account_type == 4) {
                    $the_final_Balance =  refresh_account_blance_delegate($data['account_number'], new Account(), new Delegate(), new Treasuries_transaction(), new Sales_invoice(), new services_with_orders(), false);
                } else {
                    $the_final_Balance = refresh_account_blance_General($data['account_number'], new Account(), new Treasuries_transaction(), new services_with_orders(), false);
                }
            }
            ...
            if ($data['is_account_number'] == 1) {
                refresh_account_blance_General($data['account_number'], new Account(), new Treasuries_transaction(), true);
            }
    ...
    public function printsaleswina4($id, $size)
    {
        ...
        try {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new services_with_orders(), array("*"), array("id" => $id, "come_code" => $come_code));
            ...
            $invoices_details = get_cols_where(new services_with_orders_details(), array("*"), array('services_with_orders_auto_serial' => $invoice_data['auto_serial'], 'order_type' => $invoice_data['order_type'], 'come_code' => $come_code), 'id', 'DESC');
