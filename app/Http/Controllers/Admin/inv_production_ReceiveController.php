...

{
    ...
    public function index()
    {
        ...
        $data = get_cols_where_p(new inv_production_receive(), array("*"), array("come_code" => $come_code, 'order_type' => 1), 'id', 'DESC', 10);
        if (!empty($data)) {
    ...
    public function store(Inv_production_ReceiveRequest $request)
    {
        try {
            ...
            $row = get_cols_where_row_orderby(new inv_production_receive(), array("auto_serial"), array("come_code" => $come_code, 'order_type' => 1), 'id', 'DESC');
            if (!empty($row)) {
            ...
            $data_insert['come_code'] = $come_code; //كود الشركه او كود الفرع
    ...
    {
        $come_code = auth()->user()->come_code;
        $data = get_cols_where_row(new inv_production_receive(), array("*"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
        if (empty($data)) {
    ...
    public function update($id, Inv_production_ReceiveRequest $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new inv_production_receive(), array("is_approved", "auto_serial"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($data)) {
                return redirect()->route('admin.inv_production_Receive.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            if ($data['is_approved'] == 1) {
            ...
            update(new inv_production_receive(), $data_to_update, array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
    ...
    public function delete($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new inv_production_receive(), array("is_approved", "auto_serial", "store_id", "production_lines_code"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            ...
            $flag = delete(new inv_production_receive(), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if ($flag) {
                //حذف التفاصيل المضافة علي الفاتورة - ملحوظة هي بالفعل تم حذفها من خلال العلاقة بقاعده البيانات بين الجدولين
                delete(new inv_production_receive_details(), array("inv_production_receive_auto_serial" => $parent_pill_data['auto_serial'], "come_code" => $come_code, 'order_type' => 1));
            }
    ...
    public function show($id)
    {
        try {
            ...
            $details = get_cols_where(new inv_production_receive_details(), array("*"), array('inv_production_receive_auto_serial' => $data['auto_serial'], 'order_type' => 1, 'come_code' => $come_code), 'id', 'DESC');
            if (!empty($details)) {
    ...
    public function load_modal_add_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new inv_production_receive(), array("is_approved"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($parent_pill_data)) {
    ...
    public function add_new_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $item_code = $request->item_code;
            $parent_pill_data = get_cols_where_row(new inv_production_receive(), array("is_approved", "order_date", "tax_value", "discount_value", "id"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    ...
                    if ($flag) {
                        /** update parent pill */
                        $total_detials_sum = get_sum_where(new inv_production_receive_details(), 'total_price', array("inv_production_receive_auto_serial" => $request->autoserailparent, 'order_type' => 1, 'come_code' => $come_code));
                        ...
                        update(new inv_production_receive(), $dataUpdateParent, array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
    ...
    public function reload_parent_pill(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new inv_production_receive(), array("*"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
            $data['added_by_admin'] = User::where('id', $data['added_by'])->value('name');
            $data['production_lines_name'] = Inv_production_lines::where('production_lines_code', $data['production_lines_code'])->value('name');
            $data['store_name'] = Store::where('id', $data['store_id'])->value('name');
            if ($data['updated_by'] > 0 and $data['updated_by'] != null) {
    ...
    public function reload_itemsdetials(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = get_cols_where_row(new inv_production_receive(), array("is_approved", "id"), array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($data)) {
                $details = get_cols_where(new inv_production_receive_details(), array("*"), array('inv_production_receive_auto_serial' => $auto_serial, 'order_type' => 1, 'come_code' => $come_code), 'id', 'DESC');
                if (!empty($details)) {
                    foreach ($details as $info) {
                        $info->item_card_name = Inv_itemCard::where('item_code', $info->item_code)->value('name');
                        $info->uom_name = get_field_value(new Inv_uom(), "name", array("id" => $info->uom_id));
    ...
    public function delete_details($id, $parent_id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new inv_production_receive(), array("is_approved", "auto_serial"), array("id" => $parent_id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($parent_pill_data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا حدث خطأ ما']);
            }
            ...
            if (!empty($item_row)) {
                ...
                if ($flag) {
                    /** update parent pill */
                    $total_detials_sum = get_sum_where(new inv_production_receive_details(), 'total_price', array("inv_production_receive_auto_serial" => $parent_pill_data['auto_serial'], 'order_type' => 1, 'come_code' => $come_code));
                    $dataUpdateParent['total_cost_items'] = $total_detials_sum;
                    $dataUpdateParent['total_befor_discount'] = $total_detials_sum + $parent_pill_data['tax_value'];
                    $dataUpdateParent['total_cost'] = $dataUpdateParent['total_befor_discount'] - $parent_pill_data['discount_value'];
                    $dataUpdateParent['updated_by'] = auth()->user()->id;
                    $dataUpdateParent['updated_at'] = date("Y-m-d H:i:s");
                    update(new inv_production_receive(), $dataUpdateParent, array("id" => $parent_id, "come_code" => $come_code, 'order_type' => 1));
                    return redirect()->back()
    ...
    public function load_modal_approve_invoice(Request $request)
    {
        if ($request->ajax()) {
            ...
            $counterDetails = get_count_where(new inv_production_receive_details(), array("inv_production_receive_auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
    ...
    function do_approve($auto_serial, Request $request)
    {
        ...
        $data = get_cols_where_row(new inv_production_receive(), array("total_cost_items", "is_approved", "id", "account_number", "store_id", "production_lines_code"), array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
        if (empty($data)) {
            return redirect()->route("admin.inv_production_Receive.index")->with(['error' => "عفوا غير قادر علي الوصول الي البيانات المطلوبة !!"]);
        }
        ...
        $counterDetails = get_count_where(new inv_production_receive_details(), array("inv_production_receive_auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
        if ($counterDetails == 0) {
        ...
        $flag = update(new inv_production_receive(), $dataUpdateParent, array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1, 'is_approved' => 0));
        if ($flag) {
            ...
            refresh_account_blance_ProductionLine($data['account_number'], new Account(), new Inv_production_lines(), new Treasuries_transaction(), new Services_with_orders(), new Inv_production_exchange(), new inv_production_receive(), false);
            //store move حركة المخزن
            //first Get item card data جنجيب الاصناف اللي علي الفاتورة
            $items = get_cols_where(new inv_production_receive_details(), array("*"), array("inv_production_receive_auto_serial" => $auto_serial, "come_code" => $come_code, "order_type" => 1), "id", "ASC");
            if (!empty($items)) {
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = inv_production_receive::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where($field4, $operator4, $value4)->where($field5, $operator5, $value5)->where($field6, $operator6, $value6)->where('order_type', '=', 1)->orderBy('id', 'DESC')->paginate(10);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = User::where('id', $info->added_by)->value('name');
                    $info->production_lines_name = Inv_production_lines::where('production_lines_code', $info->production_lines_code)->value('name');
    ...
    public function printsaleswina4($id, $size)
    {
        ...
        try {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new inv_production_receive(), array("*"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            ...
            $invoices_details = get_cols_where(new inv_production_receive_details(), array("*"), array('inv_production_receive_auto_serial' => $invoice_data['auto_serial'], 'order_type' => 1, 'come_code' => $come_code), 'id', 'ASC');
