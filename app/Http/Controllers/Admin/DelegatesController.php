...
use App\Http\Requests\DelegatesRequest;
...
class DelegatesController extends Controller
{
    public function index()
    {
        ...
        $data = get_cols_where_p(new Delegate(), array("*"), array("come_code" => $come_code), 'id', 'DESC', 10);
        if (!empty($data)) {
    ...
    public function store(DelegatesRequest $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            //check if not exsits for name
            $checkExists_name = get_cols_where_row(new Delegate(), array("id"), array('name' => $request->name, 'come_code' => $come_code));
            ...
            //set delegate_code
            $row = get_cols_where_row_orderby(new Delegate(), array("delegate_code"), array("come_code" => $come_code), 'id', 'DESC');
            if (!empty($row)) {
            ...
            if ($flag) {
                ...
                $delegates_parent_account_number = get_field_value(new Admin_panel_setting(), "delegate_parent_account_number", array('come_code' => $come_code));
    ...
    {
        $come_code = auth()->user()->come_code;
        $data = get_cols_where_row(new Delegate(), array("*"), array("id" => $id, "come_code" => $come_code));
    ...
    public function update($id, DelegatesUpdateRequest $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Delegate(), array("id", "account_number", "delegate_code"), array("id" => $id, "come_code" => $come_code));
            if (empty($data)) {
                return redirect()->route('admin.customers.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            $checkExists = Delegate::where(['name' => $request->name, 'come_code' => $come_code])->where('id', '!=', $id)->first();
            if ($checkExists != null) {
                return redirect()->back()
            ...
            $flag = update(new Delegate(), $data_to_update, array('id' => $id, 'come_code' => $come_code));
            if ($flag) {
                ...
                $flag = update(new Account(), $data_to_update_account, array('account_number' => $data['account_number'], 'other_table_FK' => $data['delegate_code'], 'come_code' => $come_code, 'account_type' => 4));
            }
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Delegate::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where(['come_code' => $come_code])->orderBy('id', 'DESC')->paginate(10);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = User::where('id', $info->added_by)->value('name');
                    if ($info->updated_by > 0 and $info->updated_by != null) {
            ...
            //General mirror
            $mirror['credit_sum'] = Delegate::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where("current_balance", "<", 0)->where($field3, $operator3, $value3)->where(['come_code' => $come_code])->sum('current_balance');
            $mirror['debit_sum'] = Delegate::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where("current_balance", ">", 0)->where($field3, $operator3, $value3)->where(['come_code' => $come_code])->sum('current_balance');
    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = get_cols_where_row(new Delegate(), array("*"), array("id" => $id, "come_code" => $come_code));
            if (!empty($data)) {
                $data['added_by_admin'] = get_field_value(new User(), "name", array("id" => $data['added_by'], 'come_code' => $come_code));
                if ($data['updated_by'] > 0) {
                    $data['updated_by_admin'] = get_field_value(new User(), "name", array("id" => $data['updated_by'], 'come_code' => $come_code));
                }
            }
