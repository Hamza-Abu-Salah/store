...
class Supplire_with_orderController extends Controller
{
    public function index()
    {
        $come_code = auth()->user()->come_code;
        $data = get_cols_where_p(new Supplier_with_order(), array("*"), array("come_code" => $come_code, 'order_type' => 1), 'id', 'DESC', 10);
        if (!empty($data)) {
    ...
    public function store(Suppliers_with_ordersRequest $request)
    {
        try {
            ...
            $row = get_cols_where_row_orderby(new Supplier_with_order(), array("auto_serial"), array("come_code" => $come_code, 'order_type' => 1), 'id', 'DESC');
            if (!empty($row)) {
            ...
            $id = get_field_value(new Supplier_with_order(), "id", array("auto_serial" => $data_insert['auto_serial'], "come_code" => $come_code, "order_type" => 1));
    ...
    public function show($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Supplier_with_order(), array("*"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($data)) {
            ...
            $details = get_cols_where(new Supplier_with_order_detail(), array("*"), array('suppliers_with_orders_auto_serial' => $data['auto_serial'], 'order_type' => 1, 'come_code' => $come_code), 'id', 'DESC');
            if (!empty($details)) {
    ...
    public function update($id, Suppliers_with_ordersRequest $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Supplier_with_order(), array("is_approved"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($data)) {
            ...
            update(new Supplier_with_order(), $data_to_update, array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
    ...
    public function add_new_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $item_code = $request->item_code;
            $suppliers_with_ordersData = get_cols_where_row(new Supplier_with_order(), array("is_approved", "order_date", "tax_value", "discount_value"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($suppliers_with_ordersData)) {
                if ($suppliers_with_ordersData['is_approved'] == 0) {
                    ...
                    $data_insert['created_at'] = date("Y-m-d H:i:s");
                    $data_insert['come_code'] = $come_code;
                    $flag = insert(new Supplier_with_order_detail(), $data_insert);
                    if ($flag) {
                        /** update parent pill */
                        $total_detials_sum = get_sum_where(new Supplier_with_order_detail(), 'total_price', array("suppliers_with_orders_auto_serial" => $request->autoserailparent, 'order_type' => 1, 'come_code' => $come_code));
                        ...
                        update(new Supplier_with_order(), $dataUpdateParent, array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
    ...
    public function reload_itemsdetials(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = get_cols_where_row(new Supplier_with_order(), array("is_approved", "id"), array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($data)) {
                $details = get_cols_where(new Supplier_with_order_detail(), array("*"), array('suppliers_with_orders_auto_serial' => $auto_serial, 'order_type' => 1, 'come_code' => $come_code), 'id', 'DESC');
                if (!empty($details)) {
                    foreach ($details as $info) {
                        $info->item_card_name = Inv_itemCard::where('item_code', $info->item_code)->value('name');
                        $info->uom_name = get_field_value(new Inv_uom(), "name", array("id" => $info->uom_id));
    ...
    public function reload_parent_pill(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Supplier_with_order(), array("*"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($data)) {
                $data['added_by_admin'] = User::where('id', $data['added_by'])->value('name');
                $data['supplier_name'] = Supplier::where('supplier_code', $data['supplier_code'])->value('name');
                if ($data['updated_by'] > 0 and $data['updated_by'] != null) {
    ...
    public function load_edit_item_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    $item_data_detials = get_cols_where_row(new Supplier_with_order_detail(), array("*"), array("suppliers_with_orders_auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1, 'id' => $request->id));
    ...
    public function edit_item_details(Request $request)
    {
        if ($request->ajax()) {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved", "order_date", "tax_value", "discount_value"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    ...
                    $flag = update(new Supplier_with_order_detail(), $data_to_update, array("id" => $request->id, 'come_code' => $come_code, 'order_type' => 1, 'suppliers_with_orders_auto_serial' => $request->autoserailparent));
                    if ($flag) {
                        /** update parent pill */
                        $total_detials_sum = get_sum_where(new Supplier_with_order_detail(), 'total_price', array("suppliers_with_orders_auto_serial" => $request->autoserailparent, 'order_type' => 1, 'come_code' => $come_code));
                        $dataUpdateParent['total_cost_items'] = $total_detials_sum;
    ...
    public function delete($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved", "auto_serial"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($parent_pill_data)) {
                return redirect()->back()
            ...
            $flag = delete(new Supplier_with_order(), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            if ($flag) {
                delete(new Supplier_with_order_detail(), array("suppliers_with_orders_auto_serial" => $parent_pill_data['auto_serial'], "come_code" => $come_code, 'order_type' => 1));
    ...
    public function delete_details($id, $parent_id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved", "auto_serial"), array("id" => $parent_id, "come_code" => $come_code, 'order_type' => 1));
            if (empty($parent_pill_data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا حدث خطأ ما']);
            }
            ...
            if (!empty($item_row)) {
                ...
                if ($flag) {
                    /** update parent pill */
                    $total_detials_sum = get_sum_where(new Supplier_with_order_detail(), 'total_price', array("suppliers_with_orders_auto_serial" => $parent_pill_data['auto_serial'], 'order_type' => 1, 'come_code' => $come_code));
                    $dataUpdateParent['total_cost_items'] = $total_detials_sum;
                    $dataUpdateParent['total_befor_discount'] = $total_detials_sum + $parent_pill_data['tax_value'];
                    $dataUpdateParent['total_cost'] = $dataUpdateParent['total_befor_discount'] - $parent_pill_data['discount_value'];
                    $dataUpdateParent['updated_by'] = auth()->user()->id;
                    $dataUpdateParent['updated_at'] = date("Y-m-d H:i:s");
                    update(new Supplier_with_order(), $dataUpdateParent, array("id" => $parent_id, "come_code" => $come_code, 'order_type' => 1));
                    return redirect()->back()
    ...
    public function load_modal_approve_invoice(Request $request)
    {
        if ($request->ajax()) {
            ...
            $counterDetails = get_count_where(new Supplier_with_order_detail(), array("suppliers_with_orders_auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 1));
    ...
    function do_approve($auto_serial, Request $request)
    {
        ...
        $data = get_cols_where_row(new Supplier_with_order(), array("total_cost_items", "is_approved", "id", "account_number", "store_id", "supplier_code"), array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
        if (empty($data)) {
            return redirect()->route("admin.suppliers_orders.index")->with(['error' => "عفوا غير قادر علي الوصول الي البيانات المطلوبة !!"]);
        }
        ...
        $counterDetails = get_count_where(new Supplier_with_order_detail(), array("suppliers_with_orders_auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
        if ($counterDetails == 0) {
        ...
        $flag = update(new Supplier_with_order(), $dataUpdateParent, array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 1));
        if ($flag) {
            ...
            if ($request['what_paid'] > 0) {
                ...
                $treasury_date = get_cols_where_row(new Treasuries(), array("last_isal_exhcange"), array("come_code" => $come_code, "id" => $user_shift['treasuries_id']));
            ...
            $items = get_cols_where(new Supplier_with_order_detail(), array("*"), array("suppliers_with_orders_auto_serial" => $auto_serial, "come_code" => $come_code, "order_type" => 1), "id", "ASC");
            if (!empty($items)) {
                foreach ($items as $info) {
                    ...
                    if (!empty($itemCard_Data)) {
                        ...
                        $quantityBeforMoveCurrntStore = get_sum_where(new Inv_itemcard_batches(), "quantity", array("item_code" => $info->item_code, "come_code" => $come_code, 'store_id' => $data['store_id']));
                        ...
                        $quantityAfterMoveCurrentStore = get_sum_where(new Inv_itemcard_batches(), "quantity", array("item_code" => $info->item_code, "come_code" => $come_code, 'store_id' => $data['store_id']));
                    ...
                    update(new Inv_itemCard(), $dataUpdateItemCardCosts, array("come_code" => $come_code, "item_code" => $info->item_code));
    ...
    public function printsaleswina4($id, $size)
    {
        ...
        try {
            $come_code = auth()->user()->come_code;
            $invoice_data = get_cols_where_row(new Supplier_with_order(), array("*"), array("id" => $id, "come_code" => $come_code, 'order_type' => 1));
            ...
            $invoices_details = get_cols_where(new Supplier_with_order_detail(), array("*"), array('suppliers_with_orders_auto_serial' => $invoice_data['auto_serial'], 'order_type' => 1, 'come_code' => $come_code), 'id', 'DESC');
