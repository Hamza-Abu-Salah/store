...
use App\Http\Requests\Suppliers_orders_general_return;
use App\Http\Requests\Suppliers_orders_general_returnEdit;
...
class Suppliers_with_ordersGeneralRetuen extends Controller
{
    ...
    $data = get_cols_where_p(new Supplier_with_order(), array("*"), array("come_code" => $come_code,'order_type'=>3), 'id', 'DESC', 10);
    if (!empty($data)) {
    ...
    $suupliers = get_cols_where(new Supplier(), array('supplier_code', 'name'), array('come_code' => $come_code), 'id', 'DESC');
    $stores = get_cols_where(new Store(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
    return view('admin.suppliers_orders_general_return.index', ['data' => $data, 'suupliers' => $suupliers, 'stores' => $stores]);
    }
    public function create()
    {
    $come_code = auth()->user()->come_code;
    $suupliers = get_cols_where(new Supplier(), array('supplier_code', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
    $stores = get_cols_where(new Store(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
    return view('admin.suppliers_orders_general_return.create', ['suupliers' => $suupliers, 'stores' => $stores]);
    }
    public function store(Suppliers_orders_general_return $request)
    {
    try {
    $come_code = auth()->user()->come_code;
    $supplierData = get_cols_where_row(new Supplier(), array("account_number"), array("supplier_code" => $request->supplier_code, "come_code" => $come_code));
    if (empty($supplierData)) {
    return redirect()->back()
    ->with(['error' => 'عفوا   غير قادر علي الوصول الي بيانات المورد المحدد'])
    ->withInput();
    }
    $row = get_cols_where_row_orderby(new Supplier_with_order(), array("auto_serial"), array("come_code" => $come_code,'order_type'=>3), 'id', 'DESC');
    if (!empty($row)) {
    ...
    insert(new Supplier_with_order(),$data_insert);
    $id = get_field_value(new Supplier_with_order(), "id", array("auto_serial" => $data_insert['auto_serial'], "come_code" => $come_code, "order_type" => 3));
    ...
    {
    $come_code = auth()->user()->come_code;
    $data = get_cols_where_row(new Supplier_with_order(), array("*"), array("id" => $id, "come_code" => $come_code, 'order_type' => 3));
    if (empty($data)) {
    ...
    $suupliers = get_cols_where(new Supplier(), array('supplier_code', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
    $stores = get_cols_where(new Store(), array('id', 'name'), array('come_code' => $come_code, 'active' => 1), 'id', 'DESC');
    $added_counter_details=get_count_where(new Supplier_with_order_detail(),array("come_code"=>$come_code,"order_type"=>3,"suppliers_with_orders_auto_serial"=>$data['auto_serial']));
    ...
    public function update($id, Suppliers_orders_general_returnEdit $request)
    {
    try {
    $come_code = auth()->user()->come_code;
    $data = get_cols_where_row(new Supplier_with_order(), array("is_approved","auto_serial"), array("id" => $id, "come_code" => $come_code, 'order_type' => 3));
    if (empty($data)) {
    return redirect()->route('admin.suppliers_orders_general_return.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
    }
    $supplierData = get_cols_where_row(new Supplier(), array("account_number"), array("supplier_code" => $request->supplier_code, "come_code" => $come_code));
    ...
    update(new Supplier_with_order(), $data_to_update, array("id" => $id, "come_code" => $come_code, 'order_type' => 3));
    ...
    try {
    $come_code = auth()->user()->come_code;
    $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved", "auto_serial","store_id","supplier_code"), array("id" => $id, "come_code" => $come_code, 'order_type' => 3));
    ...
    $items_details=get_cols_where(new Supplier_with_order_detail(),array("*"),array("come_code"=>$come_code,"order_type"=>3,"suppliers_with_orders_auto_serial"=>$parent_pill_data['auto_serial']));
    //حنحذف الفاتورة الاب
    $flag = delete(new Supplier_with_order(), array("id" => $id, "come_code" => $come_code, 'order_type' => 3));
    if ($flag) {
    ...
    $flagDelete=delete(new Supplier_with_order_detail(),array("come_code"=>$come_code,"order_type"=>3,"suppliers_with_orders_auto_serial"=>$parent_pill_data['auto_serial'],'id'=>$info->id));
    if($flagDelete){
    $itemCard_Data = get_cols_where_row(new Inv_itemCard(), array("uom_id", "retail_uom_quntToParent", "retail_uom_id", "does_has_retailunit","item_type"), array("come_code" => $come_code, "item_code" => $info->item_code));
    $batch_data = get_cols_where_row(new Inv_itemcard_batches(), array("quantity", "unit_cost_price", "id","production_date","expired_date"), array("come_code" => $come_code, "auto_serial" => $info->batch_auto_serial, 'store_id' => $parent_pill_data['store_id'], 'item_code' => $info->item_code));
    ...
    "quantity",
    array(
    "item_code" => $info->item_code, "come_code" => $come_code,
    ...
    new Inv_itemcard_batches(),
    "quantity",
    array("item_code" => $info->item_code, "come_code" => $come_code, 'store_id' => $parent_pill_data['store_id'])
    );
    ...
    $SupplierName = get_field_value(new Supplier(), "name", array("come_code" => $come_code, "supplier_code" => $parent_pill_data['supplier_code']));
    ...
    $details = get_cols_where(new Supplier_with_order_detail(), array("*"), array('suppliers_with_orders_auto_serial' => $data['auto_serial'], 'order_type' => 3, 'come_code' => $come_code), 'id', 'DESC');
    if (!empty($details)) {
    ...
    $data = get_cols_where_row(new Supplier_with_order(), array("is_approved","id"), array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 3));
    if (!empty($data)) {
    $details = get_cols_where(new Supplier_with_order_detail(), array("*"), array('suppliers_with_orders_auto_serial' => $auto_serial, 'order_type' => 3, 'come_code' => $come_code), 'id', 'DESC');
    if (!empty($details)) {
    foreach ($details as $info) {
    $info->item_card_name = Inv_itemCard::where('item_code', $info->item_code)->value('name');
    $info->uom_name = get_field_value(new Inv_uom(), "name", array("id" => $info->uom_id));
    ...
    if ($request->ajax()) {
    $come_code = auth()->user()->come_code;
    $data = get_cols_where_row(new Supplier_with_order(), array("*"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 3));
    if (!empty($data)) {
    $data['added_by_admin'] = User::where('id', $data['added_by'])->value('name');
    $data['supplier_name'] = Supplier::where('supplier_code', $data['supplier_code'])->value('name');
    if ($data['updated_by'] > 0 and $data['updated_by'] != null) {
    ...
    if ($request->ajax()) {
    $come_code = auth()->user()->come_code;
    $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 3));
    if (!empty($parent_pill_data)) {
    if ($parent_pill_data['is_approved'] == 0) {
    $item_data_detials = get_cols_where_row(new Supplier_with_order_detail(), array("*"), array("suppliers_with_orders_auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 3, 'id' => $request->id));
    $item_cards = get_cols_where(new Inv_itemCard(), array("name", "item_code", "item_type"), array('active' => 1, 'come_code' => $come_code), 'id', 'DESC');
    $item_card_Data = get_cols_where_row(new Inv_itemCard(), array("does_has_retailunit", "retail_uom_id", "uom_id"), array("item_code" => $item_data_detials['item_code'], "come_code" => $come_code));
    ...
    if ($request->ajax()) {
    $come_code = auth()->user()->come_code;
    $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved","store_id"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 3));
    if (!empty($parent_pill_data)) {
    if ($parent_pill_data['is_approved'] == 0) {
    $item_cards = get_cols_where(new Inv_itemCard(), array("name", "item_code", "item_type"), array('active' => 1, 'come_code' => $come_code), 'id', 'DESC');
    $stores = get_cols_where(new Store(), array('id', 'name'), array('come_code' => $come_code, 'id' => $parent_pill_data['store_id']), 'id', 'DESC');
    ...
    if ($request->ajax()) {
    $come_code = auth()->user()->come_code;
    $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved", "order_date", "tax_value", "discount_value"), array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 3));
    if (!empty($parent_pill_data)) {
    if ($parent_pill_data['is_approved'] == 0) {
    ...
    $data_to_update['updated_by'] = auth()->user()->id;
    $data_to_update['updated_at'] = date("Y-m-d H:i:s");
    $data_to_update['come_code'] = $come_code;
    $flag = update(new Supplier_with_order_detail(), $data_to_update, array("id" => $request->id, 'come_code' => $come_code, 'order_type' => 3, 'suppliers_with_orders_auto_serial' => $request->autoserailparent));
    if ($flag) {
    /** update parent pill */
    $total_detials_sum = get_sum_where(new Supplier_with_order_detail(), 'total_price', array("suppliers_with_orders_auto_serial" => $request->autoserailparent, 'order_type' => 3, 'come_code' => $come_code));
    ...
    update(new Supplier_with_order(), $dataUpdateParent, array("auto_serial" => $request->autoserailparent, "come_code" => $come_code, 'order_type' => 3));
    ...
    try {
    $come_code = auth()->user()->come_code;
    $parent_pill_data = get_cols_where_row(new Supplier_with_order(), array("is_approved", "auto_serial","store_id","supplier_code"), array("id" => $parent_id, "come_code" => $come_code, 'order_type' => 3));
    if (empty($parent_pill_data)) {
    return redirect()->back()
    ...
    $item_row = Supplier_with_order_detail::find($id);
    if (!empty($item_row)) {
    ...
    $batch_data = get_cols_where_row(new Inv_itemcard_batches(), array("quantity", "unit_cost_price", "id","production_date","expired_date"), array("come_code" => $come_code, "auto_serial" => $item_row['batch_auto_serial'], 'store_id' => $parent_pill_data['store_id'], 'item_code' => $item_row['item_code']));
    if (!empty($itemCard_Data) and !empty($batch_data)) {
    //خصم الكمية من الباتش
    //كمية الصنف بكل المخازن قبل الحركة
    $quantityBeforMove = get_sum_where(
    ...
    new Inv_itemcard_batches(),
    "quantity",
    array("item_code" => $item_row['item_code'], "come_code" => $come_code, 'store_id' => $parent_pill_data['store_id'])
    );
    //التاثير في حركة كارت الصنف
    $dataInsert_inv_itemcard_movements['inv_itemcard_movements_categories'] = 1;
    $dataInsert_inv_itemcard_movements['items_movements_types'] = 3;
    ...
    $counterDetails=get_count_where(new Supplier_with_order_detail(),array("suppliers_with_orders_auto_serial"=>$request->autoserailparent, "come_code" => $come_code, 'order_type' => 3));
    ...
    $data = get_cols_where_row(new Supplier_with_order(), array("total_cost_items", "is_approved", "id", "account_number", "store_id", "supplier_code"), array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 3));
    if (empty($data)) {
    return redirect()->route("admin.suppliers_orders.index")->with(['error' => "عفوا غير قادر علي الوصول الي البيانات المطلوبة !!"]);
    }
    if ($data['is_approved'] == 1) {
    ...
    $SupplierName = get_field_value(new Supplier(), "name", array("come_code" => $come_code, "supplier_code" => $data['supplier_code']));
    $counterDetails=get_count_where(new Supplier_with_order_detail(),array("suppliers_with_orders_auto_serial"=>$auto_serial, "come_code" => $come_code, 'order_type' => 3));
    if ($counterDetails== 0) {
    ...
    $flag = update(new Supplier_with_order(), $dataUpdateParent, array("auto_serial" => $auto_serial, "come_code" => $come_code, 'order_type' => 3));
    if ($flag) {
    ...
    refresh_account_blance_supplier($data['account_number'], new Account(), new Supplier(), new Treasuries_transaction(), new Supplier_with_order(),new Services_with_orders(), false);
    ...
    $data = Supplier_with_order::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where($field4, $operator4, $value4)->where($field5, $operator5, $value5)->where('order_type','=',1)->orderBy('id', 'DESC')->paginate(10);
    if (!empty($data)) {
    foreach ($data as $info) {
    $info->added_by_admin = User::where('id', $info->added_by)->value('name');
    $info->supplier_name = Supplier::where('supplier_code', $info->supplier_code)->value('name');
    ...
    if ($request->ajax()) {
    $come_code = auth()->user()->come_code;
    $invoice_data = get_cols_where_row(new Supplier_with_order(), array("is_approved", "order_date", "supplier_code"), array("come_code" => $come_code, "auto_serial" => $request->autoserailparent,'order_type'=>3));
    ...
    $flag_datainsert_items = insert(new Supplier_with_order_detail(), $datainsert_items, true);
    ...
    $SupplierName = get_field_value(new Supplier(), "name", array("come_code" => $come_code, "supplier_code" => $invoice_data['supplier_code']));
    ...
    {
    $come_code = auth()->user()->come_code;
    $invoice_data = get_cols_where_row(new Supplier_with_order(), array("*"), array("come_code" => $come_code, "auto_serial" => $auto_serial,'order_type'=>3));
    if (!empty($invoice_data)) {
    //first get sum of details
    $dataUpdateParent['total_cost_items'] =get_sum_where(new Supplier_with_order_detail(),"total_price",array("come_code" => $come_code, "suppliers_with_orders_auto_serial" => $auto_serial,'order_type'=>3));
    ...
    $dataUpdateParent['updated_at'] = date("Y-m-d H:i:s");
    $dataUpdateParent['updated_by'] = auth()->user()->come_code;
    update(new Supplier_with_order(), $dataUpdateParent, array("come_code" => $come_code, "auto_serial" => $auto_serial,'order_type'=>3));
    }
    }
    ...
    public function printsaleswina4($id,$size){
        ...
        try {
        $come_code = auth()->user()->come_code;
        $invoice_data = get_cols_where_row(new Supplier_with_order(), array("*"), array("id" => $id, "come_code" => $come_code, 'order_type' => 3));
        if (empty($invoice_data)) {
        return redirect()->route('admin.suppliers_orders.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
        }
        $invoice_data['added_by_admin'] = User::where('id', $invoice_data['added_by'])->value('name');
        $invoice_data['supplier_name'] = Supplier::where('supplier_code', $invoice_data['supplier_code'])->value('name');
        $invoice_data['supplier_phone'] = Supplier::where('supplier_code', $invoice_data['supplier_code'])->value('phones');
        $invoice_data['store_name'] = Store::where('id', $invoice_data['store_id'])->value('name');
        $invoices_details = get_cols_where(new Supplier_with_order_detail(), array("*"), array('suppliers_with_orders_auto_serial' => $invoice_data['auto_serial'], 'order_type' => 3, 'come_code' => $come_code), 'id', 'DESC');
