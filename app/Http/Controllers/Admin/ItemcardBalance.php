...
class ItemcardBalance extends Controller
{
    public function index()
    {
        //get all itemscars ordery by quantity
        $come_code = auth()->user()->come_code;
        $allitemscardData = get_cols_where_p(new Inv_itemCard(), array("id", "name", "item_code", "item_type", "does_has_retailunit", "retail_uom_id", "uom_id", "retail_uom_quntToParent", "All_QUENTITY"), array("come_code" => $come_code), "id", "ASC", 10);
        if (!empty($allitemscardData)) {
            foreach ($allitemscardData as $info) {
                ...
                $info->allBathces = get_cols_where_order2(new Inv_itemcard_batches(), array("*"), array("come_code" => $come_code, "item_code" => $info->item_code), 'store_id', 'ASC', 'quantity', 'DESC');
                if (!empty($info->allBathces)) {
                    foreach ($info->allBathces as $Det) {
                        $Det->store_name = get_field_value(new Store(), "name", array("come_code" => $come_code, "id" => $Det->store_id));
                        if ($info->does_has_retailunit == 1) {
        ...
        $itemCardsSearch = get_cols_where(new Inv_itemCard(), array("item_code", "name"), array("come_code" => $come_code), 'id', 'ASC');
        $storesSearch = get_cols_where(new Store(), array("id", "name"), array("come_code" => $come_code), 'id', 'ASC');
