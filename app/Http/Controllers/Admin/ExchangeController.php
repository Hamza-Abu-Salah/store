...
class ExchangeController extends Controller
{
    ...
    public function store(Exchange_transactionRequest $request)
    {
        try {
            ...
            $treasury_date = get_cols_where_row(new Treasuries(), array("last_isal_exhcange"), array("come_code" => $come_code, "id" => $request->treasuries_id));
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $totalExchangeInSearch = Treasuries_transaction::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where($field4, $operator4, $value4)->where($field5, $operator5, $value5)->where($field6, $operator6, $value6)
