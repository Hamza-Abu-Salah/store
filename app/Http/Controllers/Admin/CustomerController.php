...
use App\Http\Requests\Customer_editRequest;
use App\Http\Requests\CustomerRequest;
use App\Models\Account;
use App\Models\Admin_panel_setting;
use App\Models\User_panel_setting;
...
class CustomerController extends Controller
{
    public function index()
    {
        ...
        $data = get_cols_where_p(new Customer(), array("*"), array("come_code" => $come_code), 'id', 'DESC', 10);
        if (!empty($data)) {
    ...
    public function store(CustomerRequest $request)
    {
        try {
            ...
            $checkExists_name = get_cols_where_row(new Customer(), array("id"), array('name' => $request->name, 'come_code' => $come_code));
            ...
            //set customer code
            $row = get_cols_where_row_orderby(new Customer(), array("customer_code"), array("come_code" => $come_code), 'id', 'DESC');
            if (!empty($row)) {
            ...
            if ($flag) {
                ...
                $customer_parent_account_number = get_field_value(new Admin_panel_setting(), "customer_parent_account_number", array('come_code' => $come_code));
    ...
    {
        $come_code = auth()->user()->come_code;
        $data = get_cols_where_row(new Customer(), array("*"), array("id" => $id, "come_code" => $come_code));
        return view('admin.customers.edit', ['data' => $data]);
    }
    public function update($id, Customer_editRequest $request)
    {
        try {
            $come_code = auth()->user()->come_code;
            $data = get_cols_where_row(new Customer(), array("id", "account_number", "customer_code"), array("id" => $id, "come_code" => $come_code));
            if (empty($data)) {
                return redirect()->route('admin.customers.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            $checkExists = Customer::where(['name' => $request->name, 'come_code' => $come_code])->where('id', '!=', $id)->first();
            if ($checkExists != null) {
                return redirect()->back()
            ...
            $flag = update(new Customer(), $data_to_update, array('id' => $id, 'come_code' => $come_code));
            if ($flag) {
                ...
                $flag = update(new Account(), $data_to_update_account, array('account_number' => $data['account_number'], 'other_table_FK' => $data['customer_code'], 'come_code' => $come_code, 'account_type' => 3));
            }
    ...
    public function delete($id)
    {
        try {
            $come_code = auth()->user()->come_code;
            $item_row = get_cols_where_row(new Customer(), array("name"), array("id" => $id, "come_code" => $come_code));
            if (!empty($item_row)) {
                $flag = delete(new Customer(), ["id" => $id, "come_code" => $come_code]);
                if ($flag) {
                    return redirect()->back()
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Customer::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where(['come_code' => $come_code])->orderBy('id', 'DESC')->paginate(10);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = User::where('id', $info->added_by)->value('name');
                    if ($info->updated_by > 0 and $info->updated_by != null) {
            ...
            //General mirror
            $mirror['credit_sum'] = Customer::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where("current_balance", "<", 0)->where($field3, $operator3, $value3)->where(['come_code' => $come_code])->sum('current_balance');
            $mirror['debit_sum'] = Customer::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where("current_balance", ">", 0)->where($field3, $operator3, $value3)->where(['come_code' => $come_code])->sum('current_balance');
