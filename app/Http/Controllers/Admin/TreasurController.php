...
use App\Http\Requests\Add_terasuries_delivery;
use App\Http\Requests\treasuriesRequest;
...
class TreasurController extends Controller
{
    ...
    public function store(TreasuriesRequest $request)
    {
        ...
        try {
            ...
            //check if not exsits
            $checkExists = Treasuries::where(['name' => $request->name, 'come_code' => $come_code])->first();
            if ($checkExists == null) {
                if ($request->is_master == 1) {
                    $checkExists_isMaster = Treasuries::where(['is_master' => 1, 'come_code' => $come_code])->first();
    ...
    public function update($id, TreasuriesRequest $request)
    {
        try {
            ...
            $checkExists = Treasuries::where(['name' => $request->name, 'come_code' => $come_code])->where('id', '!=', $id)->first();
            if ($checkExists != null) {
                return redirect()->back()
                    ->with(['error' => 'عفوا اسم الخزنة مسجل من قبل'])
                    ->withInput();
            ...
            if ($request->is_master == 1) {
                $checkExists_isMaster = Treasuries::where(['is_master' => 1, 'come_code' => $come_code])->where('id', '!=', $id)->first();
                if ($checkExists_isMaster != null) {
                    return redirect()->back()
                        ->with(['error' => 'عفوا هناك خزنة رئيسية بالفعل مسجلة من قبل لايمكن ان يكون هناك اكثر من خزنة رئيسية'])
                        ->withInput();
            ...
            Treasuries::where(['id' => $id, 'come_code' => $come_code])->update($data_to_update);
    ...
    public function Add_treasuries_delivery($id)
    {
        ...
        try {
            ...
            $Treasuries = Treasuries::select('id', 'name')->where(['come_code' => $come_code, 'active' => 1])->get();



            return view("admin.treasuries.Add_treasuries_delevery", ['data' => $data, 'Treasuries' => $Treasuries]);
        } catch (\Exception $ex) {
    ...
    public function store_treasuries_delivery($id,Add_terasuries_delivery $request)
    {
        ...
        try {
            ...
            $checkExists = Treasuries_delivery::where(['treasuries_id' => $id, 'treasuries_can_delivery_id' => $request->treasuries_can_delivery_id, 'come_code' => $come_code])->first();
            if ($checkExists != null) {
                return redirect()->back()
            ...
            $data_insert_details['come_code'] = $come_code;
