...
function get_user_shift($Admins_Shifts, $Treasuries = null, $Treasuries_transactions = null)
{
    ...
    $data = $Admins_Shifts::select("treasuries_id", "shift_code")->where(["come_code" => $come_code, "user_id" => auth()->user()->id, "is_finished" => 0])->first();
    if (!empty($data)) {
        $data['name'] = $Treasuries::where(["id" => $data["treasuries_id"], "come_code" => $come_code])->value("name");
        $data['balance'] = $Treasuries_transactions::where(["shift_code" => $data["shift_code"], "come_code" => $come_code])->sum("money");
    }
    return $data;
}
...
function refresh_account_blance_supplier($account_number = null, $AccountModel = null, $SupplierModel = null, $treasuries_transactionsModel = null, $suppliers_with_ordersModel = null, $ServicesOrdersModel = null, $returnFlag = false)
{
    ...
    $AccountData =  $AccountModel::select("start_balance", "account_type")->where(["come_code" => $come_code, "account_number" => $account_number])->first();
    //لو مورد
    if ($AccountData['account_type'] == 2) {
        //صافي مجموع المشتريات والمرتجعات للمورد
        $the_net_in_suppliers_with_orders = $suppliers_with_ordersModel::where(["come_code" => $come_code, "account_number" => $account_number])->sum("money_for_account");
        //صافي حركة النقديه بالخزن علي حساب المورد
        $the_net_in_treasuries_transactions = $treasuries_transactionsModel::where(["come_code" => $come_code, "account_number" => $account_number])->sum("money_for_account");
        //صافي حركة فواتير الخدمات الخارجية والداخلية المتعلقه بالحساب المالي للمورد
        $the_net_in_services_orders = $ServicesOrdersModel::where(["come_code" => $come_code, "account_number" => $account_number, 'is_account_number' => 1])->sum("money_for_account");
        ...
        $AccountModel::where(["come_code" => $come_code, "account_number" => $account_number])->update($dataToUpdateAccount);
        $dataToUpdateSupplier['current_balance'] = $the_final_Balance;
        //update in Accounts حندث جدول الموردين  بحقل المورد
        $SupplierModel::where(["come_code" => $come_code, "account_number" => $account_number])->update($dataToUpdateSupplier);
        if ($returnFlag) {
...
function refresh_account_blance_delegate($account_number = null, $AccountModel = null, $delgateModel = null, $treasuries_transactionsModel = null, $SalesinvoiceModel = null, $ServicesOrdersModel = null, $returnFlag = true)
{
    $come_code = auth()->user()->come_code;
    //حنجيب الرصيد الافتتاحي  للحساب اول المده لحظة تكويده
    $AccountData =  $AccountModel::select("start_balance", "account_type", "other_table_FK")->where(["come_code" => $come_code, "account_number" => $account_number])->first();
    //لو مندوب
    if (!empty($AccountData)) {
        if ($AccountData['account_type'] == 4) {
            //صافي مجموع عموله المندوب بالمبيعات
            $the_net_sales_invoicesForDelegate = $SalesinvoiceModel::where(["come_code" => $come_code, "delegate_code" => $AccountData['other_table_FK']])->sum("delegate_commission_value");
            ...
            $delgateModel::where(["come_code" => $come_code, "account_number" => $account_number])->update($dataToUpdateDelgate);
            if ($returnFlag) {
                return $the_final_Balance;
            }
        }
...
function refresh_account_blance_customer($account_number = null, $AccountModel = null, $customerModel = null, $treasuries_transactionsModel = null, $SalesinvoiceModel = null, $SalesReturnModel = null, $ServicesOrdersModel = null, $returnFlag = false)
{
    ...
    if ($AccountData['account_type'] == 3) {
        ...
        $the_net_sales_invoicesForCustomer = $SalesinvoiceModel::where(["come_code" => $come_code, "account_number" => $account_number])->sum("money_for_account");
        //    صافي  مرتجع المبيعات بس لما نعمله
        $the_net_sales_invoicesReturnForCustomer = $SalesReturnModel::where(["come_code" => $come_code, "account_number" => $account_number])->sum("money_for_account");
        ...
        $customerModel::where(["come_code" => $come_code, "account_number" => $account_number])->update($dataToUpdateSupplier);
        if ($returnFlag) {
            return $the_final_Balance;
        }
    }
...
function do_update_itemCardQuantity($Inv_itemCard = null, $item_code = null, $Inv_itemcard_batches = null, $does_has_retailunit = null, $retail_uom_quntToParent = null)
{
    ...
    $allQuantityINBatches =  $sum = $Inv_itemcard_batches::where(["come_code" => $come_code, "item_code" => $item_code])->sum("quantity");
    ...
    update($Inv_itemCard, $DataToUpdateItemCardQuantity, array("come_code" => $come_code, "item_code" => $item_code));
}
...
function refresh_account_blance_ProductionLine($account_number = null, $AccountModel = null, $ProductionLineModel = null, $treasuries_transactionsModel = null, $ServicesOrdersModel = null, $inv_production_exchangeModel = null, $inv_production_receiveModel = null, $returnFlag = false)
{
    ...
    if ($AccountData['account_type'] == 5) {
        ...
        $the_net_in_inv_production_exchange = $inv_production_exchangeModel::where(["come_code" => $come_code, "account_number" => $account_number])->sum("money_for_account");
        $the_net_inv_production_receive = $inv_production_receiveModel::where(["come_code" => $come_code, "account_number" => $account_number])->sum("money_for_account");
        ...
        $the_net_Services_inv_productionLine = $ServicesOrdersModel::where(["come_code" => $come_code, "account_number" => $account_number, 'is_account_number' => 1])->sum("money_for_account");
        ...
        //update in Accounts حندث جدول الموردين  بحقل المورد
        $ProductionLineModel::where(["come_code" => $come_code, "account_number" => $account_number])->update($dataToUpdateLine);
        if ($returnFlag) {
            return $the_final_Balance;
        }
    }
